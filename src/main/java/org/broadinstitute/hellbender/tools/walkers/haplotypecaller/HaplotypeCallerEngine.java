    ...
    private List<VariantContext> referenceModelForNoVariation(final AssemblyRegion region, final boolean needsToBeFinalized, final List<VariantContext> VCpriors) {
        if ( emitReferenceConfidence() ) {
            ...
            if ( needsToBeFinalized ) {
                AssemblyBasedCallerUtils.finalizeRegion(region, hcArgs.errorCorrectReads, hcArgs.dontUseSoftClippedBases, minTailQuality, readsHeader, samplesList, hcArgs.correctOverlappingBaseQualities);
            }
